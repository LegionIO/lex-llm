    ...
    class Bedrock
      ...
      module Chat
        ...
        def render_message_content(msg)
          if msg.content.is_a?(LexLLM::Content::Raw)
        ...
        def render_tool_result_content(content)
          return render_raw_tool_result_content(content.value) if content.is_a?(LexLLM::Content::Raw)
          return [{ json: content }] if content.is_a?(Hash) || content.is_a?(Array)
          return render_content_tool_result_content(content) if content.is_a?(LexLLM::Content)
        ...
        def render_tool(tool)
          input_schema = tool.params_schema || LexLLM::Tool::SchemaDefinition.from_parameters(tool.parameters)&.json_schema
          ...
          LexLLM::Utils.deep_merge(tool_spec, tool.provider_params)
        end
        ...
        def render_additional_model_request_fields(thinking)
          ...
          fields = LexLLM::Utils.deep_merge(fields, reasoning_fields) if reasoning_fields
        ...
        def build_output_config(schema)
          ...
          cleaned = LexLLM::Utils.deep_dup(schema[:schema])
