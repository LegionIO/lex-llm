  ...
  module Providers
    ...
    class Bedrock < Provider
      ...
      def normalize_params(params, model:)
        normalized = LexLLM::Utils.deep_symbolize_keys(params || {})
        ...
        if !top_k.nil? && model_supports_top_k?(model)
          additional_fields = LexLLM::Utils.deep_merge(additional_fields, { top_k: top_k })
        end
      ...
      def api_payload(payload)
        cleaned = LexLLM::Utils.deep_symbolize_keys(LexLLM::Utils.deep_dup(payload))
