      ...
      module Temperature
        ...
        def normalize(temperature, model_id)
          if model_id.match?(/^(o\d|gpt-5)/) && !temperature.nil? && !temperature_close_to_one?(temperature)
            LexLLM.logger.debug { "Model #{model_id} requires temperature=1.0, setting that instead." }
            1.0
          elsif model_id.include?('-search')
            LexLLM.logger.debug { "Model #{model_id} does not accept temperature parameter, removing" }
            nil
          else
            temperature
          end
