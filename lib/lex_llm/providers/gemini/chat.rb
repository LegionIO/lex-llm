    ...
    class Gemini
      ...
      module Chat
        ...
        def convert_schema_to_gemini(schema)
          ...
          # Extract inner schema if wrapper format (e.g., from LexLLM::Schema.to_json_schema)
        ...
        def build_json_schema(schema)
          normalized = LexLLM::Utils.deep_dup(schema[:schema])
          normalized.delete(:strict)
          normalized.delete('strict')
          LexLLM::Utils.deep_stringify_keys(normalized)
        end
        ...
        class GeminiSchema
          def initialize(schema)
            @raw_schema = LexLLM::Utils.deep_dup(schema)
            @definitions = {}
          end
          ...
          def merge_definitions(raw_defs)
            ...
            @definitions = if definitions.empty?
                             ...
                             LexLLM::Utils.deep_merge(definitions, symbolized)
                           end
          end
          ...
          def resolve_reference(schema, visited_refs)
            ...
            merged = LexLLM::Utils.deep_merge(referenced, overrides)
          ...
          def lookup_definition(ref) # rubocop:disable Metrics/PerceivedComplexity
            ...
            current ? LexLLM::Utils.deep_dup(current) : nil
          end
          ...
          def normalize_any_of(schema)
            ...
            options = Array(any_of).map { |option| LexLLM::Utils.deep_symbolize_keys(option) }
            nullables, non_null = options.partition { |option| schema_type(option) == 'null' }

            base = LexLLM::Utils.deep_symbolize_keys(non_null.first || { type: 'string' })
