    ...
    class Gemini # rubocop:disable Style/Documentation
      ...
      module Media
        ...
        def format_content(content)
          return content.value if content.is_a?(LexLLM::Content::Raw)
      ...
      def build_inline_attachment(inline_data, index)
        ...
        LexLLM::Attachment.new(io, filename:)
      rescue ArgumentError => e
        LexLLM.logger.warn "Failed to decode Gemini inline data attachment: #{e.message}"
        nil
      end
      ...
      def build_file_attachment(file_data, index)
        ...
        LexLLM::Attachment.new(uri, filename:)
      end
