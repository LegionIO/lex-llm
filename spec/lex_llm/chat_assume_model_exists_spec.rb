...
RSpec.describe LexLLM::Chat do
  ...
  describe '#assume_model_exists' do
    ...
    let!(:original_models) { LexLLM::Models.instance.all.dup }

    # Clean up the model registry after each test
    after do
      LexLLM::Models.instance.instance_variable_set(:@models, original_models)
      # Also reset the singleton to ensure clean state
      LexLLM::Models.instance_variable_set(:@instance, nil)
    end
    ...
    it 'skips registry validation when assuming model exists' do
      expect(LexLLM::Models).not_to receive(:find) # rubocop:disable RSpec/MessageSpies
    ...
    it 'works with LexLLM.chat convenience method' do
      chat = LexLLM.chat(
        model: custom_model,
        provider: provider,
        assume_model_exists: true
    ...
    it 'works with models not in registry but available in API' do
      ...
      LexLLM::Models.instance.instance_variable_set(:@models, filtered_models)

      # Should raise error when not assuming existence
      expect do
        LexLLM.chat(model: real_model)
