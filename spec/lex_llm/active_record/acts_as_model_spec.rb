...
RSpec.describe LexLLM::ActiveRecord::ActsAs do
  ...
  describe 'acts_as_model' do
    ...
    describe 'model persistence' do
      it 'syncs models from LexLLM registry' do
        allow(LexLLM.models).to receive(:refresh!)
        allow(LexLLM.models).to receive(:all).and_return([model_info])
    ...
    describe 'conversions' do
      ...
      it 'converts to Model::Info with to_llm' do
        result = model.to_llm
        expect(result).to be_a(LexLLM::Model::Info)
    ...
    describe 'model registry integration' do
      ...
      it 'finds models from database' do
        ...
        expect(found).to be_a(LexLLM::Model::Info)
    ...
    describe 'chat integration with model association' do
      ...
      before do
        ...
        # Reload models from database so LexLLM.models knows about them
        LexLLM.models.load_from_database!
      end
      ...
      it 'fails when model does not exist' do
        expect { chat_class.create!(model_id: 'non-existent') }.to raise_error(LexLLM::ModelNotFoundError)
      end
  ...
  def with_temp_model_registry_file
    original_file = LexLLM.config.model_registry_file
    temp_file = Tempfile.new(['lex_llm_models', '.json'])
    temp_file.write(
      [
        {
    ...
    LexLLM.configure { |config| config.model_registry_file = temp_file.path }
    yield
  ensure
    LexLLM.configure { |config| config.model_registry_file = original_file }
    temp_file&.close!
  end
end
