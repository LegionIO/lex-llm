...
RSpec.describe LexLLM::ActiveRecord::ActsAs do
  ...
  class Calculator < LexLLM::Tool # rubocop:disable Lint/ConstantDefinitionInBlock,RSpec/LeakyConstantDeclaration
  ...
  describe 'default model' do
    it 'uses config default when no model specified' do
      ...
      expect(chat.reload.model_id).to eq(LexLLM.config.default_model)
      expect(chat.messages.count).to eq(2)
    end
  end
  ...
  describe 'tool functionality' do
    it 'supports with_tools for multiple tools' do
      ...
      weather_tool = Class.new(LexLLM::Tool) do
    ...
    it 'handles halt mechanism in tools' do
      ...
      stub_const('HaltingTool', Class.new(LexLLM::Tool) do
      ...
      allow_any_instance_of(HaltingTool).to receive(:execute).and_return( # rubocop:disable RSpec/AnyInstance
        LexLLM::Tool::Halt.new('Halted response')
      )
  ...
  describe 'raw content support' do
    ...
    it 'persists raw content blocks separately from plain text' do
      ...
      raw_block = LexLLM::Providers::Anthropic::Content.new('Cache me once', cache: true)
  ...
  describe 'error handling' do
    it 'destroys empty assistant messages on API failure' do
      ...
      allow_any_instance_of(LexLLM::Chat).to receive(:complete).and_raise(LexLLM::Error) # rubocop:disable RSpec/AnyInstance

      expect { chat.ask('This will fail') }.to raise_error(LexLLM::Error)
  ...
  describe 'custom configurations' do
    ...
    describe 'to_llm conversion' do
      it 'correctly converts custom messages to LexLLM format' do
        bot_chat = Assistants::BotChat.create!(model: model)
        ...
        expect(llm_message).to be_a(LexLLM::Message)
      ...
      it 'correctly converts tool calls' do
        ...
        expect(llm_tool_call).to be_a(LexLLM::ToolCall)
  ...
  describe 'attachment handling' do
    ...
    it 'converts ActiveStorage attachments to LexLLM Content' do
      ...
      llm_message = message.to_llm
      expect(llm_message.content).to be_a(LexLLM::Content)
