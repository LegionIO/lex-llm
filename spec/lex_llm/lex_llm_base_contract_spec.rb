...
RSpec.describe LexLLM do
  ...
  it 'loads without registering concrete provider gems' do
    ...
    expect(LexLLM::Provider.providers).to eq({})
    expect(LexLLM::Routing::ModelOffering).to be_a(Class)
  end
  ...
  it 'lets a provider gem register options and satisfy chat through the shared API' do
    ...
    expect(LexLLM::Configuration.options).to include(:fake_llm_api_key, :fake_llm_api_base)
  end
