...
RSpec.describe LexLLM::Chat do # rubocop:disable RSpec/MultipleMemoizedHelpers
  ...
  describe 'text models' do # rubocop:disable RSpec/MultipleMemoizedHelpers
    CHAT_MODELS.each do |model_info|
      ...
      it "#{provider}/#{model} can understand text" do
        ...
        expect(response.content).not_to include('LexLLM::Content')
        expect(chat.messages.first.content).to be_a(LexLLM::Content)
        ...
        expect(response.content).to be_present
        expect(response.content).not_to include('LexLLM::Content')
        expect(chat.messages[2].content).to be_a(LexLLM::Content)
  ...
  describe 'URL attachment handling' do # rubocop:disable RSpec/MultipleMemoizedHelpers
    it 'handles URL MIME type detection without ArgumentError' do
      attachment = LexLLM::Attachment.new(image_url)

      expect(attachment.mime_type).to be_present
    end

    it 'creates content with URL attachments' do
      content = LexLLM::Content.new('Describe this image', image_url)

      expect(content.attachments).not_to be_empty
      expect(content.attachments.first).to be_a(LexLLM::Attachment)
    end
  ...
  describe 'IO attachment handling' do # rubocop:disable RSpec/MultipleMemoizedHelpers
    it 'handles StringIO objects' do
      ...
      attachment = LexLLM::Attachment.new(string_io)
    ...
    it 'handles StringIO objects with filename' do
      ...
      attachment = LexLLM::Attachment.new(string_io, filename: 'test.txt')
    ...
    it 'handles Tempfile objects' do
      ...
      attachment = LexLLM::Attachment.new(tempfile)
    ...
    it 'handles File objects' do
      File.open(text_path, 'r') do |file|
        attachment = LexLLM::Attachment.new(file)
    ...
    it 'reads path attachments in binary mode' do
      attachment = LexLLM::Attachment.new(image_path)
    ...
    it 'handles ActionDispatch::Http::UploadedFile' do
      ...
      attachment = LexLLM::Attachment.new(uploaded_file)
    ...
    it 'creates content with IO attachments' do
      ...
      content = LexLLM::Content.new('Check this')
    ...
    it 'ignores blank attachment placeholders in arrays' do
      ...
      expect { LexLLM::Content.new('Check this', ['', uploaded_file]) }.not_to raise_error

      content = LexLLM::Content.new('Check this', ['', uploaded_file])
    ...
    it 'ignores nil-only attachment entries' do
      content = LexLLM::Content.new('Check this', [nil, nil])
