...
RSpec.describe LexLLM::ErrorMiddleware do
  describe '.parse_error' do
    let(:provider) { instance_double(LexLLM::Provider, parse_error: 'provider error') }
    ...
    it 'maps 502 to ServiceUnavailableError' do
      ...
      end.to raise_error(LexLLM::ServiceUnavailableError)
    end
    ...
    it 'maps context-length-like 429 errors to ContextLengthExceededError' do
      ...
      provider = instance_double(LexLLM::Provider, parse_error: 'Request too large for model')

      expect do
        described_class.parse_error(provider: provider, response: response)
      end.to raise_error(LexLLM::ContextLengthExceededError)
    end
    ...
    it 'keeps regular 429 errors as RateLimitError' do
      ...
      provider = instance_double(LexLLM::Provider, parse_error: 'Rate limit exceeded')

      expect do
        described_class.parse_error(provider: provider, response: response)
      end.to raise_error(LexLLM::RateLimitError)
    end
    ...
    it 'maps context-length-like 400 errors to ContextLengthExceededError' do
      ...
      provider = instance_double(LexLLM::Provider, parse_error: msg)
    ...
    it 'keeps regular 400 errors as BadRequestError' do
      ...
      provider = instance_double(LexLLM::Provider, parse_error: 'Invalid model specified')

      expect do
        described_class.parse_error(provider: provider, response: response)
      end.to raise_error(LexLLM::BadRequestError)
    end
  end
end
