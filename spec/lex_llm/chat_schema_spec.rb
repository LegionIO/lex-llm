...
# Define a test schema class for testing LexLLM::Schema instances
class PersonSchemaClass < LexLLM::Schema
  string :name
  number :age
end
  ...
  describe '#with_schema' do
    ...
    STRUCTURED_OUTPUT_MODELS.each do |model_info|
      ...
      context "with #{provider}/#{model}" do
        let(:chat) { LexLLM.chat(model: model, provider: provider) }
        ...
        it 'accepts LexLLM::Schema class instances and returns structured output' do
          skip 'Model does not support structured output' unless chat.model.structured_output?
    ...
    describe 'schema name sanitization' do
      it 'sanitizes :: from namespaced LexLLM::Schema class names' do
        namespaced_schema = stub_const('MyApp::Nested::TestSchema', Class.new(LexLLM::Schema) do
          string :name
        end)
    ...
    describe 'schema with tool calls' do
      before do
        stub_const('SchemaToolTestWeather', Class.new(LexLLM::Tool) do
      ...
      it 'does not parse tool-call response content as JSON when schema is set' do
        chat = LexLLM.chat.with_tool(SchemaToolTestWeather).with_schema(person_schema)
