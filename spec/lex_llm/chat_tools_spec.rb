...
def supports_functions?(provider, model)
  return false if LexLLM::Provider.providers[provider]&.local?

  model_info = LexLLM.models.find(model)
  skip "#{model} doesn't support function calling" unless model_info&.supports_functions?
end

RSpec.describe LexLLM::Chat do
  include_context 'with configured LexLLM'

  class Weather < LexLLM::Tool # rubocop:disable Lint/ConstantDefinitionInBlock,RSpec/LeakyConstantDeclaration
  ...
  class BestLanguageToLearn < LexLLM::Tool # rubocop:disable Lint/ConstantDefinitionInBlock,RSpec/LeakyConstantDeclaration
  ...
  class BrokenTool < LexLLM::Tool # rubocop:disable Lint/ConstantDefinitionInBlock,RSpec/LeakyConstantDeclaration
  ...
  class DiceRoll < LexLLM::Tool # rubocop:disable Lint/ConstantDefinitionInBlock,RSpec/LeakyConstantDeclaration
  ...
  class HaltingTool < LexLLM::Tool # rubocop:disable Lint/ConstantDefinitionInBlock,RSpec/LeakyConstantDeclaration
  ...
  class HandoffTool < LexLLM::Tool # rubocop:disable Lint/ConstantDefinitionInBlock,RSpec/LeakyConstantDeclaration
  ...
  class ContentReturningTool < LexLLM::Tool # rubocop:disable Lint/ConstantDefinitionInBlock,RSpec/LeakyConstantDeclaration
  ...
  class ParamsTool < LexLLM::Tool # rubocop:disable Lint/ConstantDefinitionInBlock,RSpec/LeakyConstantDeclaration
  ...
  class ArrayParamsTool < LexLLM::Tool # rubocop:disable Lint/ConstantDefinitionInBlock,RSpec/LeakyConstantDeclaration
  ...
  class AnyOfParamsTool < LexLLM::Tool # rubocop:disable Lint/ConstantDefinitionInBlock,RSpec/LeakyConstantDeclaration
  ...
  class ObjectParamsTool < LexLLM::Tool # rubocop:disable Lint/ConstantDefinitionInBlock,RSpec/LeakyConstantDeclaration
  ...
  describe 'tool choice normalization' do
    it 'accepts a tool class for choice' do
      chat = LexLLM.chat.with_tool(Weather, choice: Weather)
  ...
  describe 'function calling' do
    CHAT_MODELS.each do |model_info|
      ...
      it "#{provider}/#{model} can use tools" do
        ...
        chat = LexLLM.chat(model: model, provider: provider)
                     .with_tool(Weather)
      ...
      it "#{provider}/#{model} deals with non-existent tool calls" do
        hallucinated_tool_call = LexLLM::ToolCall.new(
          id: 'call_1',
          name: 'list_tools',
          arguments: {}
        )
    ...
    CHAT_MODELS.each do |model_info| # rubocop:disable Style/CombinableLoops
      ...
      it "#{provider}/#{model} can use tools without parameters in multi-turn streaming conversations" do
        ...
        expect(chunks.first).to be_a(LexLLM::Chunk)
        expect(response.content).to include('Ruby')
    ...
    CHAT_MODELS.each do |model_info| # rubocop:disable Style/CombinableLoops
      ...
      it "#{provider}/#{model} can handle with_params" do
        ...
        allow(provider_instance).to receive(:sync_response) do |_connection, payload, _|
          ...
          LexLLM::Message.new(role: :assistant, content: 'ok')
        end
    ...
    CHAT_MODELS.each do |model_info| # rubocop:disable Style/CombinableLoops
      ...
      it "#{provider}/#{model} handles array params" do
        ...
        expect(response).to be_a(LexLLM::Tool::Halt)
  ...
  describe 'content object support' do
    CHAT_MODELS.each do |model_info|
      ...
      it "#{provider}/#{model} preserves Content objects returned from tools" do
        ...
        expect(tool_message.content).to be_a(LexLLM::Content)
        expect(tool_message.content.text).to eq('Processed: test data')
        expect(tool_message.content.attachments).not_to be_empty
        expect(tool_message.content.attachments.first).to be_a(LexLLM::Attachment)
  ...
  describe 'halt functionality' do
    it 'returns Halt object when tool halts' do
      chat = LexLLM.chat.with_tool(HaltingTool)
    ...
    it 'returns sub-agent result through halt' do
      chat = LexLLM.chat.with_tool(HandoffTool)
      provider = chat.instance_variable_get(:@provider)
      tool_call = LexLLM::ToolCall.new(
        id: 'call_1',
        name: 'handoff',
  ...
  describe 'tool choice and calls control' do
    CHAT_MODELS.each do |model_info|
      ...
      it "#{provider}/#{model} respects choice: :none" do
        unless LexLLM::Provider.providers[provider]&.local?
          model_info = LexLLM.models.find(model)
          skip "#{model} doesn't support function calling" unless model_info&.supports_functions?
        end

        provider_class = provider ? LexLLM::Provider.providers[provider.to_sym] : nil
        ...
        expect(response).to be_a(LexLLM::Message)
      end
  ...
  describe 'error handling' do
    it 'raises an error when tool execution fails' do
      chat = LexLLM.chat.with_tool(BrokenTool)
