...
RSpec.describe LexLLM::Models do
  ...
  describe 'refresh models output structure' do
    ...
    it 'returns models with consistent structure' do
      ...
      expect(models.all).to all(be_a(LexLLM::Model::Info))

      models.all.each do |model|
        expect(model.capabilities).to be_an(Array)
        expect(model.modalities).to be_a(LexLLM::Model::Modalities)
        expect(model.pricing).to be_a(LexLLM::Model::Pricing)
      end
    end
