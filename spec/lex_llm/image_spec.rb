...
RSpec.describe LexLLM::Image do
  ...
  def invalid_content_type_url
    'https://legionio.dev/favicon.svg'
  end

  def missing_remote_image_url
    'https://legionio.dev/some-asset-that-does-not-exist.png'
  end
  ...
  describe 'basic functionality' do
    IMAGE_GENERATION_MODELS.each do |model_info|
      ...
      it "#{provider}/#{model} can paint images" do
        image = LexLLM.paint('a siamese cat', model: model, provider: provider)
      ...
      it "#{provider}/#{model} supports custom sizes" do
        image = LexLLM.paint('a siamese cat', size: '1792x1024', model: model, provider: provider)
  ...
  describe 'edit functionality' do
    context 'with local files' do
      it 'supports image edits with a valid local PNG' do
        image = LexLLM.paint(prompt, with: image_path, model: model)
      ...
      it 'rejects edits with a non-png local file' do
        expect do
          LexLLM.paint(prompt, with: audio_path, model: model)
        end.to raise_error(LexLLM::UnsupportedAttachmentError, /only supports image attachments/)
      end
    ...
    context 'with remote URLs' do
      it 'rejects edits with a URL having invalid content type' do
        expect do
          LexLLM.paint(prompt, with: invalid_content_type_url, model: model)
        end.to raise_error(LexLLM::BadRequestError, /unsupported mimetype/)
      end

      it 'rejects edits with a URL that returns 404' do
        expect do
          LexLLM.paint(prompt, with: missing_remote_image_url, model: model)
