...
RSpec.describe LexLLM::Providers::Anthropic::Chat do
  describe '.render_payload' do
    let(:model) { instance_double(LexLLM::Model::Info, id: 'claude-sonnet-4-5', max_tokens: nil) }

    it 'embeds raw system content blocks unchanged' do
      system_raw = LexLLM::Providers::Anthropic::Content.new(
        'avoid greetings',
        cache_control: { type: 'ephemeral' }
      )

      system_message = LexLLM::Message.new(role: :system, content: system_raw)
      user_message = LexLLM::Message.new(role: :user, content: 'Hello there')
    ...
    it 'includes output_config when schema is provided' do
      ...
      user_message = LexLLM::Message.new(role: :user, content: 'Hello')
