...
RSpec.describe LexLLM::Providers::Anthropic::Content do
  describe '.new' do
    it 'builds a raw text block' do
      raw = described_class.new('Hello world')

      expect(raw).to be_a(LexLLM::Content::Raw)
  ...
  describe 'chat integration' do
    ...
    it 'passes raw content through to the Anthropic provider' do
      chat = LexLLM.chat(model: 'claude-sonnet-4-5', provider: :anthropic, assume_model_exists: true)
      ...
      allow(provider).to receive(:complete) do |messages, **_options, &|
        ...
        expect(user_message.content).to be_a(LexLLM::Content::Raw)
        expect(user_message.content.value.first[:cache_control]).to eq(type: 'ephemeral')

        LexLLM::Message.new(role: :assistant, content: 'Fine')
      end
