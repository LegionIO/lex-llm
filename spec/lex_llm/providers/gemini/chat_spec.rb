...
RSpec.describe LexLLM::Providers::Gemini::Chat do
  include_context 'with configured LexLLM'

  # Create a test object that includes the module to access private methods
  let(:test_obj) do
    Object.new.tap do |obj|
      obj.extend(LexLLM::Providers::Gemini::Media)
      obj.extend(LexLLM::Providers::Gemini::Tools)
  ...
  describe '#convert_schema_to_gemini' do
    it 'extracts inner schema from wrapper format' do
      # Simulate what LexLLM::Schema.to_json_schema returns
      schema = {
        name: 'PersonSchema',
        schema: {
          type: 'object',
  ...
  describe '#render_payload' do
    ...
    it 'uses responseJsonSchema for Gemini 2.5 models' do
      model = instance_double(LexLLM::Model::Info, id: 'gemini-2.5-flash', metadata: {})
    ...
    it 'unwraps wrapper schemas for responseJsonSchema' do
      model = instance_double(LexLLM::Model::Info, id: 'gemini-3.0-pro', metadata: {})
      wrapped_schema = {
        name: 'PersonSchema',
        schema: {
          type: 'object',
    ...
    it 'falls back to responseSchema for non-2.5 models' do
      model = instance_double(LexLLM::Model::Info, id: 'gemini-2.0-flash', metadata: {})
  ...
  describe '#format_messages' do
    it 'groups consecutive tool responses into a single user message with multiple function responses' do
      messages = [
        LexLLM::Message.new(role: :user, content: 'Question?'),
        LexLLM::Message.new(
          role: :assistant,
          content: '',
          tool_calls: {
            'call_1' => LexLLM::ToolCall.new(id: 'call_1', name: 'weather', arguments: {}),
            'call_2' => LexLLM::ToolCall.new(id: 'call_2', name: 'best_language_to_learn', arguments: {})
          }
        ),
        LexLLM::Message.new(role: :tool, content: 'Sunny', tool_call_id: 'call_1'),
        LexLLM::Message.new(role: :tool, content: 'Ruby', tool_call_id: 'call_2')
      ]
  ...
  describe '#parse_completion_response' do
    it 'keeps thought-only parts out of assistant content' do
      ...
      provider = LexLLM::Providers::Gemini.new(LexLLM.config)
  ...
  it 'correctly sums candidatesTokenCount and thoughtsTokenCount' do
    chat = LexLLM.chat(model: 'gemini-2.5-flash', provider: :gemini)
