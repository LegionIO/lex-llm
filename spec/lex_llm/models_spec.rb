...
RSpec.describe LexLLM::Models do
  ...
  describe 'filtering and chaining' do
    it 'excludes slash-based vertexai models from the registry' do
      models = described_class.new(
        [
          LexLLM::Model::Info.new(id: 'deepseek-ai/deepseek-v3.1-maas', name: 'DeepSeek', provider: 'vertexai'),
          LexLLM::Model::Info.new(id: 'gemini-2.5-flash', name: 'Gemini', provider: 'vertexai'),
          LexLLM::Model::Info.new(id: 'deepseek-ai/deepseek-v3.1-maas', name: 'DeepSeek', provider: 'openrouter')
        ]
      )
    ...
    it 'filters models by provider' do
      openai_models = LexLLM.models.by_provider('openai')
    ...
    it 'chains filters in any order with same result' do
      ...
      openai_chat_models = LexLLM.models.by_provider('openai').chat_models
      chat_openai_models = LexLLM.models.chat_models.by_provider('openai')
    ...
    it 'supports Enumerable methods' do
      ...
      provider_counts = LexLLM.models.group_by(&:provider)
    ...
    it 'filters by vision support' do
      vision_models = LexLLM.models.select(&:supports_vision?)
      expect(vision_models).to all(have_attributes(supports_vision?: true))
    end

    it 'filters by video support' do
      video_models = LexLLM.models.select(&:supports_video?)
  ...
  describe 'finding models' do
    it 'finds models by ID' do
      ...
      model_id = LexLLM.config.default_model
      model = LexLLM.models.find(model_id)
      expect(model.id).to eq(model_id)

      # Find a model with chaining
      if LexLLM.models.by_provider('openai').chat_models.any?
        openai_chat_id = LexLLM.models.by_provider('openai').chat_models.first.id
        found = LexLLM.models.by_provider('openai').find(openai_chat_id)
    ...
    it 'raises ModelNotFoundError for unknown models' do
      expect do
        LexLLM.models.find('nonexistent-model-12345')
  ...
  describe '#find' do
    it 'prioritizes exact matches over aliases' do
      chat_model = LexLLM.chat(model: 'gemini-2.0-flash')
      expect(chat_model.model.id).to eq('gemini-2.0-flash')

      chat_model = LexLLM.chat(model: 'gemini-2.0-flash', provider: 'gemini')
      expect(chat_model.model.id).to eq('gemini-2.0-flash')

      # Only use alias when exact match isn't found
      chat_model = LexLLM.chat(model: 'claude-3-5-haiku')
    ...
    it 'prefers bedrock region-resolved inference profile IDs over exact unprefixed IDs' do
      unprefixed = LexLLM::Model::Info.new(
        id: 'meta.llama4-maverick-17b-instruct-v1:0',
        name: 'Llama 4 Maverick',
        provider: 'bedrock',
        metadata: {}
      )
      prefixed = LexLLM::Model::Info.new(
      ...
      allow(LexLLM).to receive(:config).and_return(
        instance_double(LexLLM::Configuration, bedrock_region: 'us-west-2')
      )
  ...
  describe '#refresh!' do
    ...
    it 'updates models and returns a chainable Models instance' do
      ...
      chat_models = LexLLM.models.refresh!.chat_models
    ...
    it 'works as a class method too' do
      ...
      expect(LexLLM.models.all.size).to be_positive
    end
  end
  ...
  describe '#embedding_models' do
    it 'filters to models that are embedding-capable' do
      embedding_models = LexLLM.models.embedding_models
  ...
  describe '#chat_models' do
    it 'excludes models with non-text output modalities' do
      chat_models = LexLLM.models.chat_models
  ...
  describe '#audio_models' do
    it 'filters to models that are audio-capable' do
      audio_models = LexLLM.models.audio_models
  ...
  describe '#image_models' do
    it 'filters to models that are image-capable' do
      image_models = LexLLM.models.image_models
  ...
  describe '#by_family' do
    it 'filters models by family' do
      ...
      family = LexLLM.models.all.first.family
      family_models = LexLLM.models.by_family(family)
  ...
  describe '#resolve' do
    it 'delegates to the class method when called on instance' do
      ...
      model_info, provider_instance = LexLLM.models.resolve(model_id, provider: provider)

      expect(model_info).to be_a(LexLLM::Model::Info)
      expect(model_info.id).to eq(model_id)
      expect(model_info.provider).to eq(provider)
      expect(provider_instance).to be_a(LexLLM::Provider)
    end

    it 'resolves model without provider' do
      model_id = 'gpt-4o'

      model_info, provider_instance = LexLLM.models.resolve(model_id)
    ...
    it 'resolves with assume_exists option' do
      ...
      model_info, provider_instance = LexLLM.models.resolve(
        model_id,
        provider: provider,
        assume_exists: true
      )
