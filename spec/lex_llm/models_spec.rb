...
RSpec.describe LexLLM::Models do
  ...
  it 'supports provider-specific model id normalization through provider hooks' do
    ...
    LexLLM::Provider.register(:normalizing_fake, provider)
    normalized = LexLLM::Model::Info.new(
      id: 'fake-chat-model',
      name: 'Normalized',
  ...
  it 'raises when a model exists but its provider extension is not registered' do
    ...
    expect { described_class.resolve('orphan-model') }.to raise_error(LexLLM::Error, /Unknown provider/)
  end
