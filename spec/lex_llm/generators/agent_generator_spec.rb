...
require 'spec_helper'
require 'fileutils'
require 'generators/lex_llm/agent/agent_generator'
require_relative '../../support/generator_test_helpers'

RSpec.describe LexLLM::Generators::AgentGenerator, :generator, type: :generator do
  ...
  it 'creates an agent class and instructions prompt in the convention path' do
    within_test_app(app_path) do
      output, status = run_rails_generate('lex_llm:agent', 'Support')
      ...
      expect(agent_class).to include('class SupportAgent < LexLLM::Agent')
