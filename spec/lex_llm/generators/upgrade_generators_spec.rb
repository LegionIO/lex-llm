...
require 'spec_helper'
require 'fileutils'
require 'generators/lex_llm/upgrade_to_v1_7/upgrade_to_v1_7_generator'
require 'generators/lex_llm/upgrade_to_v1_9/upgrade_to_v1_9_generator'
require 'generators/lex_llm/upgrade_to_v1_14/upgrade_to_v1_14_generator'
require_relative '../../support/generator_test_helpers'

RSpec.describe 'LexLLM upgrade generators', :generator, type: :generator do # rubocop:disable RSpec/DescribeClass
  ...
  describe 'with default model names' do
    ...
    it 'creates the expected v1.7 migration files' do
      within_test_app(app_path) do
        ...
        expect(migrations_containing('migrate_to_lex_llm_model_references').any?).to be true
      end
    end
    ...
    it 'updates existing initializer when present' do
      within_test_app(app_path) do
        initializer = File.read('config/initializers/lex_llm.rb')
        expect(initializer).to include('LexLLM.configure')
    ...
    it 'creates the expected v1.9 migration file' do
      within_test_app(app_path) do
        migration_path = migrations_containing('add_lex_llm_v1_9_columns').first
    ...
    it 'creates the expected v1.14 migration file' do
      within_test_app(app_path) do
        migration_path = migrations_containing('add_lex_llm_v1_14_columns').first
        expect(migration_path).not_to be_nil
  ...
  describe 'with custom model mappings' do
    ...
    it 'respects model mapping args in generated v1.7 migration content' do
      within_test_app(app_path) do
        migration_path = migrations_containing('migrate_to_lex_llm_model_references').first
        expect(migration_path).not_to be_nil
