...
require 'spec_helper'
require 'fileutils'
require 'generators/lex_llm/schema/schema_generator'
require_relative '../../support/generator_test_helpers'

RSpec.describe LexLLM::Generators::SchemaGenerator, :generator, type: :generator do
  ...
  it 'creates a schema class in the convention path' do
    within_test_app(app_path) do
      output, status = run_rails_generate('lex_llm:schema', 'Product')
      expect(status.success?).to be(true), output

      expect(File.exist?('app/schemas/product.rb')).to be true

      schema_class = File.read('app/schemas/product.rb')
      expect(schema_class).to include('class ProductSchema < LexLLM::Schema')
