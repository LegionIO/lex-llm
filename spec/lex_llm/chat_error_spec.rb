    ...
    CHAT_MODELS.each do |model_info|
      ...
      context "with #{provider}/#{model}" do
        ...
        before do
          ...
          LexLLM::Provider.remote_providers.each_key do |slug|
            # Skip vertexai as it uses OAuth, not API keys
            next if slug == :vertexai

            LexLLM.config.public_send("#{slug}_api_key=", 'invalid-key')
          end
        end

        it 'raises appropriate auth error' do
          skip('Only valid for remote providers') if LexLLM::Provider.providers[provider].local?
          skip('Vertex AI uses OAuth, not API keys') if provider == :vertexai
          expect { chat.ask('Hello') }.to raise_error do |error|
            expect(error).to be_a(LexLLM::Error)
            expect(error.class.ancestors).to include(LexLLM::Error)
    ...
    CHAT_MODELS.each do |model_info|
      ...
      context "#{provider}/#{model}" do # rubocop:disable RSpec/ContextWording
        ...
        it 'handles context length exceeded errors' do
          if LexLLM::Provider.providers[provider]&.local?
          ...
          expect { chat.ask('Hi') }.to raise_error(LexLLM::Error) do |e|
